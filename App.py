...
st.set_page_config(
    ...
    page_icon="KineTexLogo.png",
    #layout="wide"
)

members_db=os.getenv("MONGO_DATABASE")
                ...
                if st.button("📩 Insert"):
                    ...
                    for member in df.to_dict("records"):
                        try:
                            ...
                            member["img_url"] = "" if pd.isna(img_url) else ex.gimageconvert(str(img_url),"I"+str(member["roll"])+"_"+member["time"])
...
def updatemember(roll,new_name,new_roll,new_pos,new_img_url,new_prof_url,new_bio,new_phn_no):
    try:
        ...
        if roll!=new_roll and collection.find_one({"roll": new_roll}):
            return False
        new_img_url=ex.gimageconvert(new_img_url,"I"+str(new_roll)+"_"+ex.getDateTime())
        update={
            "$set":
                {
                    "name": new_name,
...
def addmember(new_name, new_roll, new_pos, new_img_url,new_prof_url,new_bio,new_phn_no):
    try:
        ...
        if collection.find_one({"roll": new_roll}):
            return False
        ist=ex.getDateTime()
        new_img_url=ex.gimageconvert(new_img_url,"I"+str(new_roll)+"_"+ist)
        member = {
            "name": new_name,
            "roll": new_roll,
            "pos": new_pos,
...
def login():
    ...
    uri = f"mongodb+srv://{st.session_state.username}:{st.session_state.password}@{os.getenv('MONGO_CLUSTER')}.mongodb.net/"
